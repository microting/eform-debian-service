...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("MicrotingService")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation',
            'Microsoft.Extensions.Identity.Stores', 'Microsoft.Extensions.Localization.Abstractions',
            'Microsoft.AspNetCore.Mvc.Razor.RuntimeCompilation', 'Microsoft.Extensions.Identity.Stores',
            'Microsoft.Extensions.Localization.Abstractions', 'System.Private.ServiceModel',
            'Microsoft.Extensions.DependencyModel', 'System.ComponentModel.Composition', 'Sentry', 'QuestPDF'
        ]
        process_repository('MicrotingService.csproj', packages, 'eform-debian-service')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("..")
            run_command(f"github_changelog_generator -u microting -p eform-debian-service -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
            run_command(["git", "add", "CHANGELOG.md"])
            run_command(["git", "commit", "-m", "Updating changelog"])
            run_command(["git", "push"])
            run_command("git checkout stable", shell=True)
            run_command("git merge master", shell=True)
